...
def ggml(dataloader,a=10,l=0.1,k=5,lr=0.01,norm='fro',max_iterations=30,diagonal=False,random_init=True,fixed_init=None,verbose=True,save_path=None,save_i_iterations=None,plot_i_iterations=None,dataset=None,n_threads=64):
    ...
    #Iterations
        ...
        if dataset is not None and plot_i_iterations is not None and i%plot_i_iterations==0:
            print(f"Compute all OT distances after {i} iterations")
            D = dataset.compute_OT_on_dists(w = w_theta.clone().detach().numpy())   

    return w_theta.clone().detach().numpy()


def triplet_loss(triplet,w,alpha=torch.scalar_tensor(0.1),n_threads=8):
    X_i,X_j,X_k = triplet
...
def compute_dists(A,B,theta):
    ...
    for i,x in enumerate(A):
        for j,y in enumerate(B):
            D[i,j] = mahalanobis_distance(x,y,W = theta) # np.linalg.norm(np.squeeze(np.dot(theta,x))-np.squeeze((np.dot(theta,y)))) 
    return D

def mahalanobis_distance(x,y,W=None,M=None):
    if M is None:
        assert W is not None, "Atleast one of M or W need to be passed to Mahalanobis distance"
        M = np.dot(W,W.transpose())
        print(f"Dist on M {distance.mahalanobis(x,y,np.linalg.inv(M))}")
        print(f"Dist on W {np.linalg.norm(np.squeeze(np.dot(W,x))-np.squeeze((np.dot(W,y))))}")
...
def ggml_notorch(distributions,labels,alpha=0.1,num_iter=1000,temp=0.01,threshold=0.01,lambda_=0.001):
    ...
    for s in range(num_iter):
        ...
        for (i,j,k) in tqdm(triplets):
            ...
            loss = np.sum(np.multiply(dist_ij,pi_ij)) - np.sum(alpha * np.multiply(dist_jk,pi_jk)) + lambda_ * np.linalg.norm(np.dot(theta,theta.transpose()),ord=1) #min 
        ...
        print(f"{s}. iteration | avg loss {np.average(losses)} min {np.min(losses)} max {np.max(losses)}| new avg loss {np.average(losses_new)} | loss change {np.average(losses_diff)} | step {theta_update_scale}")
        print("Theta New:")
        print(theta)
        #plot_ellipses(theta)
        plot_ellipses(np.dot(theta,np.transpose(theta)))
