...
neighbor_t = [1,2,3,4,5] #,5,7
rank_k = [5,10,50,100,500,1000] #,8433] #,10,25,50,100,200] 
shared_init_over_runs = (len(rank_k) == 1) #for fixed k we can init all param combinations jointly

alphas =  [10**0] #[10**i for i in range(-1,4)]# #,100] #,10,50,100]
lambdas = [10**3]   #[10**i for i in range(-1,4)] #[0.1,1,10,100]#,100] #,10,50,100]
...
####
#Dataset
n_cells = 1000 #Cause normal dataloader only works for identical sizes
pca_c = None
...
for t in neighbor_t:
    ...
    train_dataset_t = DataLoader(training_data[t], batch_size=64, shuffle=True)
    for a in alphas:
        for l in lambdas:
                for k in rank_k:
