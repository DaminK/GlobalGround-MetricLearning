    ...
    for a in alphas:
        for l in lambdas:
                ...
                #test_data = CustomSyntheticDataset(distribution_size=n, class_means = means, offsets = offsets, shared_means_x=shared_means_x, shared_means_y=shared_means_y, plot=True, varying_size=False)
                        ...
                        train_dataset[(a,l,t,k)] = DataLoader(training_data[t], batch_size=128, shuffle=True)
...
for a,l,t,k in train_dataset.keys():
    ...
    #Iterations
        ...
        if i%save_every_i_iterations==0:
            np.save(f"/home/kuehn/ot_metric_learning/damin-ggml/data/results/learned_parameters/myocard_infarct/GGML/theta_{a}_{l}_{t}_{k}_iter{i}.npy",w_theta.clone().detach().numpy())


            print(f"saved under: /home/kuehn/ot_metric_learning/damin-ggml/data/results/learned_parameters/myocard_infarct/GGML/theta_{a}_{l}_{t}_{k}_iter{i}.npy")
